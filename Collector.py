import sys
from styles import (start_end_styles, level_button_styles, stylize_level_button)
from AbstractWidget import AbstractWidget
from PySide6.QtCore import Qt, QTimer, QSize
from PySide6.QtWidgets import (QWidget, QApplication, QLabel, QPushButton, QVBoxLayout,QHBoxLayout, QSplitter, QSpacerItem,QSizePolicy)
...
class TestAndCollectPage(AbstractWidget):
    def __init__(self, switch_page_callback, folder_path, connections):
        ...
        self.enable(self.start_button, start_end_styles['enabled'])
        ...
        self.disable(self.end_button, start_end_styles['disabled'])
        layout.addWidget(self.end_button, alignment=Qt.AlignmentFlag.AlignCenter)

        self.setLayout(layout)

    def start(self):
        self.disable(self.start_button, start_end_styles['disabled'])
        self.enable(self.end_button, start_end_styles['enabled'])
...
class TesterPage(AbstractWidget):
...
class CollectorPage(AbstractWidget):
    def __init__(self, folder_path, connections):
        super().__init__()
        ...
        self.title.setFont(super().set_font_size(30))
    ...
    def disable_all_level_buttons(self):
        for button, button_properties in self.level_buttons.items():
            ...
            self.disable(button, styles=stylize_level_button(background_color, text_color, enabled=False))

    def enable_all_level_buttons(self):
        for button, button_properties in self.level_buttons.items():
            text_color = button_properties['text_color']
            background_color = button_properties['background_color']
            self.enable(button, styles=stylize_level_button(background_color, text_color, enabled=True))
    ...
    def start(self):
        ...
        # # initiate the datalogger
        # self.data_logger = DataLogger(self.folder_path, "Fatigue", self.connections)
